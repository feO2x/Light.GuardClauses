...
public static class MustNotBeTests
{
    ...
    {
        Action act = () => x.MustNotBe(y, comparisonType);

        act.Should().Throw<ValuesEqualException>()
           .And.Message.Should().Contain(
                $"{nameof(x)} must not be equal to {y.ToStringOrNull()}, but it actually is {x.ToStringOrNull()}"
            );
    }
    ...
    public static void ValuesNotEqual(string x, string y, StringComparison comparisonType) =>
        x.MustNotBe(y, comparisonType).Should().Be(x);
    ...
    [Fact]
    public static void CustomExceptionStringsNotEqual() =>
        "Foo".MustNotBe("Bar", StringComparison.Ordinal, (_, _) => null).Should().Be("Foo");

    [Fact]
    public static void CustomExceptionIgnoreWhiteSpaceStringsNotEqual() =>
        "Foo".MustNotBe("Bar", StringComparisonType.OrdinalIgnoreWhiteSpace, (_, _, _) => null).Should().Be("Foo");

    [Fact]
    public static void CustomMessage() =>
        Test.CustomMessage<ValuesEqualException>(
            message => "Foo".MustNotBe("Foo", StringComparison.CurrentCulture, message: message)
        );

    [Fact]
    public static void CustomMessageIgnoreWhiteSpace() =>
        Test.CustomMessage<ValuesEqualException>(
            message => "Bar".MustNotBe(
                "bar  ",
                StringComparisonType.OrdinalIgnoreCaseIgnoreWhiteSpace,
                message: message
            )
